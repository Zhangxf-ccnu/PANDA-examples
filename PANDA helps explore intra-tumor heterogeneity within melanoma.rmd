...
sc_results <- sc_train(sc_counts, sc_labels, n_archetypes_vec = 10, n_cores = 6, save_dir = "./PANDA_results/sc_results")
```
...
### Variations within the same cell type
...
We perform Gene Ontology (GO) enrichment analysis on these marker genes to explore the different roles of B cells in different regions within the melanoma tissue.
...
We investigate the cell-cell communication between immune cells and malignant cells within the melanoma-rich region.

#### Identification of interaction-changed genes

We first identify potential genes in malignant cells that may be influenced by immune cells by looking for genes in malignant cells whose expression changed with the proportion of immune cells within the melanoma-rich regions based on the estimated malignant cell-specific gene expression and cell type composition for each spot. Here, immune cells encompass B cells, macrophages, NK cells, and T cells. Our focus is on spots containing a proportion of malignant cells greater than 0.1 in the melanoma-rich region.
...
#### Inference of active ligand-target links

Using the identified potential interaction-changed genes as target genes within malignant cells, we explore the intercellular communication between immune cells and malignant cells by predicting links between ligands expressed by immune cells and the target genes of interest within malignant cells. 

We implement this procedure by [NicheNet](https://github.com/saeyslab/nichenetr). The `lr_network_human_21122021.rds` and `ligand_target_matrix_nsga2r_final.rds` used here can be downloaded from [Zenodo](https://zenodo.org/records/7074291).

```{r load_nichenet, eval=FALSE}
library(nichenetr)
library(tidyverse)
library(circlize)
...
Note that here we only consider B cells, macrophages and T cells, excluding NK cells due to their rarity. We focus on spots containing a proportion of malignant cells greater than $0.1$ and a proportion of at least one immune cell type greater than $0.1$ within the melanoma-rich region.

```{r keep_spots_immune, eval=FALSE}
keep_spots_immune <- keep_spots[apply(proportion[keep_spots, c("B", "T", "Macro")], 1, max) > 0.1]
...
![](./docs/communication_spot_labels.png){width=50%}

We first define expressed genes in the immune cells (sender) and malignant cells (receiver). Genes are considered to be expressed within one cell type if their log2-transformed mean TPM (transcript per kilobase million) values are greater than $4$, following the same criterion as in the NicheNet tutorial. The mean TPM value for each cell type is calculated based on the cell-type-specific gene expression of spots with the corresponding proportion greater than 0.1 within the focused spots.

```{r define_expressed_genes, eval=FALSE}
...
expressed_genes_B <- keep_genes[log2(colMeans(mu_list[["B"]][keep_spots_B, , drop = FALSE] * 1e6) + 1) >= 4]
expressed_genes_T <- keep_genes[log2(colMeans(mu_list[["T"]][keep_spots_T, , drop = FALSE] * 1e6) + 1) >= 4]
expressed_genes_Macro <- keep_genes[log2(colMeans(mu_list[["Macro"]][keep_spots_Macro, , drop = FALSE] * 1e6) + 1) >= 4]

expressed_genes_Malignant <- keep_genes[log2(colMeans(mu_list[["Malignant"]][keep_spots_immune, , drop = FALSE] * 1e6) + 1) >= 4]
```

The gene set of interest comprises the identified potential interaction-changed genes, encompassing both significantly positively correlated genes and significantly negatively correlated genes.

```{r geneset_oi, eval=FALSE}
geneset_oi_pos <- intersect(convert_alias_to_symbols(sig_positive_genes, "human", verbose = FALSE), rownames(ligand_target_matrix))
geneset_oi_neg <- intersect(convert_alias_to_symbols(sig_negative_genes, "human", verbose = FALSE), rownames(ligand_target_matrix))

geneset_oi <- c(geneset_oi_pos, geneset_oi_neg)
...
The background of genes encompasses all the analyzed genes (genes used for archetypal analysis).

```{r background_genes, eval=FALSE}
background_expressed_genes <- intersect(convert_alias_to_symbols(test_genes, "human", verbose = FALSE), rownames(ligand_target_matrix))
```

The potential ligands are defined as ligands expressed in at least one immune cell type and for which at least one putative receptor is expressed in malignant cells.

```{r potential_ligands, eval=FALSE}
ligands <- lr_network %>% pull(from) %>% unique()
expressed_ligands_B <- intersect(ligands, expressed_genes_B)
expressed_ligands_T <- intersect(ligands, expressed_genes_T)
expressed_ligands_Macro <- intersect(ligands, expressed_genes_Macro)
expressed_ligands <- Reduce("union", list(expressed_ligands_B, expressed_ligands_T, expressed_ligands_Macro))
...
We then perform NicheNet's ligand activity analysis.

```{r ligand_activity_analysis, eval=FALSE}
ligand_activities <- predict_ligand_activities(geneset = geneset_oi, background_expressed_genes = background_expressed_genes, ligand_target_matrix = ligand_target_matrix, potential_ligands = potential_ligands)

best_upstream_ligands <- ligand_activities %>% arrange(-aupr_corrected) %>% pull(test_ligand)
```

We assign these ligands to one or more immune cell types based on their expression levels in each immune cell type. The assignment criterion is that the log2-transformed mean TPM (transcript per kilobase million) value is greater than $4$.

```{r ligand_assignment, eval=FALSE}
intersect(best_upstream_ligands, expressed_ligands_B)
intersect(best_upstream_ligands, expressed_ligands_T)
intersect(best_upstream_ligands, expressed_ligands_Macro)
...
B_specific_ligands <- ligand_expression_tbl %>% filter(B >= 4 & T < 4 & Macro < 4) %>% pull(ligand)
T_specific_ligands <- ligand_expression_tbl %>% filter(B < 4 & T >= 4 & Macro < 4) %>% pull(ligand)
Macro_specific_ligands <- ligand_expression_tbl %>% filter(B < 4 & T < 4 & Macro >= 4) %>% pull(ligand)
B_T_ligands <- ligand_expression_tbl %>% filter(B >= 4 & T >= 4 & Macro < 4) %>% pull(ligand)
B_Macro_ligands <- ligand_expression_tbl %>% filter(B >= 4 & T < 4 & Macro >= 4) %>% pull(ligand)
T_Macro_ligands <- ligand_expression_tbl %>% filter(B < 4 & T >= 4 & Macro >= 4) %>% pull(ligand)
B_T_Macro_ligands <- ligand_expression_tbl %>% filter(B >= 4 & T >= 4 & Macro >= 4) %>% pull(ligand)
...
When inferring target genes of the ligands, we consider the top 100 targets of at least one ligand according to the prior model.

```{r target_infer, eval=FALSE}
...
We visualize the active ligand-target links in a circos plot. We only display links with a weight higher than 0.8 quantile.

```{r circos_plot, eval=FALSE}
...
order_targets_pos = hclust_targets_pos$labels[hclust_targets_pos$order]
...
order_targets_neg = hclust_targets_neg$labels[hclust_targets_neg$order]
order_targets <- c(order_targets_pos, order_targets_neg)
...
![](./docs/ligand_target_circos.png)

We visualize the expression of ligands in immune cells, the regulatory potential between the ligands and target genes, the expression of target genes in malignant cells and the Pearson correlation coefficient between the expression of the target genes and the proportion of immune cells in a combined heatmap.

```{r combined_heatmap, eval=FALSE}
library(cowplot)
library(ggpubr)
...
![](./docs/ligand_target_matrix.png)

We perform GO enrichment analysis and KEGG enrichment analysis on the identified active target genes in the significantly positively correlated gene group and in the significantly negatively correlated gene group to uncover diverse biological functions associated with each gene group.

```{r active_target_genes, eval=FALSE}
active_target_pos <- active_ligand_target_links_df %>% filter(target_type == "Positive") %>% pull(target) %>% unique()
active_target_neg <- active_ligand_target_links_df %>% filter(target_type == "Negative") %>% pull(target) %>% unique()
```

GO enrichment analysis:

```{r enrichGO_target, eval=FALSE}
...
![](./docs/enrichGO_active_target_pos.png){width=50%}
![](./docs/enrichGO_active_target_neg.png){width=50%}

KEGG enrichment analysis:

```{r enrichKEGG_target, eval=FALSE}
...
![](./docs/enrichKEGG_active_target_pos.png){width=50%}

![](./docs/enrichKEGG_active_target_neg.png){width=50%}
